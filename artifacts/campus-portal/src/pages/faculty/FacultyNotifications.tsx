import {
  useListFacultyNotifications,
  useMarkFacultyNotificationRead,
  getListFacultyNotificationsQueryKey,
...
export default function FacultyNotifications() {
  ...
  const { data, isLoading } = useListFacultyNotifications();
  const markRead = useMarkFacultyNotificationRead();

  const onRead = (id: string) => {
    markRead.mutate({ notificationId: id }, {
      onSuccess: () => qc.invalidateQueries({ queryKey: getListFacultyNotificationsQueryKey() }),
    });
  };

  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <PageHeader title="Notifications" description="System alerts and student updates." />
      <Card>
        <CardContent className="p-0">
          {isLoading ? (
            <div className="p-6 space-y-3">{[1,2,3].map(i => <Skeleton key={i} className="h-16 w-full" />)}</div>
          ) : !data || data.length === 0 ? (
            <EmptyState icon={Bell} title="No notifications yet" description="You'll see updates here when students form groups or send replies." />
          ) : (
